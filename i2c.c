...
*
*   Invoking i2c results in a read or write I2C transfer.  Options include the
...
*      mostly to keep consistency with the spincl tool usage.
*
...
*
*           Bias DAC (MCP4725) at some voltage
...
uint8_t peripheral_address = 0x00;
uint32_t len = 0;
...
int comparse(int argc, char **argv) {
    ...
    while (argnum < argc && argv[argnum][0] == '-') {
        ...
        switch (argv[argnum][1]) {
            ...
            case 's':  // Peripheral address
                peripheral_address = atoi(argv[argnum]+2);
                break;
...
int showusage(int errcode) {
    ...
    printf("\n");
    printf("    len: The number of bytes to be transmitted or received in decimal.\n");
    printf("    The maximum number of bytes allowed is %d\n", MAX_LEN);
    printf("\n");
    printf("    -s: Peripheral address in decimal\n");
    printf("\n");
    printf("\n");
...
int main(int argc, char **argv) {
    ...
    bcm2835_i2c_setSlaveAddress(peripheral_address);
    bcm2835_i2c_setClockDivider(clk_div);
    fprintf(stderr, "Clock divider set to: %d\n", clk_div);
    fprintf(stderr, "len set to: %d\n", len);
    fprintf(stderr, "Peripheral address set to: %d\n", peripheral_address);
